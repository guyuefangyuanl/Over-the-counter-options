...
# 本地缓存文件路径（云托管环境使用 /tmp 目录）
_STOCK_CODES_CACHE_FILE = os.environ.get("STOCK_CODES_CACHE_PATH", "/tmp/.stock_codes_cache.json")
...
def _fetch_stock_codes_from_sina() -> List[str]:
    ...
    logger.info("[全量更新] 正在从新浪财经获取沪深A股列表...")
    ...
    while True:
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.error(f"[全量更新] 新浪API响应异常: {resp.status_code}")
                break

            data = resp.json()
            if not data:
                logger.info(f"[全量更新] 新浪API第{page}页返回空数据，停止分页")
                break
            ...
            logger.info(f"[全量更新] 新浪API第{page}页获取 {len(page_codes)} 只，总计 {len(codes)}")
            ...
            logger.error(f"[全量更新] 新浪API第{page}页请求超时")
            break
        except requests.exceptions.RequestException as e:
            logger.error(f"[全量更新] 新浪API第{page}页请求失败: {e}")
            break
        except Exception as e:
            logger.error(f"[全量更新] 新浪API第{page}页处理异常: {type(e).__name__}: {e}")
            break
    ...
    logger.info(f"[全量更新] 新浪API总共获取到 {len(valid_codes)} 只有效股票代码")
    return valid_codes


def _fetch_stock_codes_from_eastmoney() -> List[str]:
    """从东方财富 API 获取所有 A 股股票代码（备用方案）"""
    codes: List[str] = []
    ...
    for config in market_configs:
        ...
        while True:
            ...
            try:
                ...
                logger.info(f"[全量更新] {name} 第{page}页 API响应状态: {resp.status_code}")
                ...
                if data and "data" in data and data["data"] and "diff" in data["data"]:
                    ...
                    logger.info(f"[全量更新] {name} 第{page}页获取 {len(page_codes)} 只，总计 {len(market_codes)}/{total}")
                ...
                logger.error(f"[全量更新] {name} 第{page}页 API 请求超时")
                break
            except requests.exceptions.RequestException as e:
                logger.error(f"[全量更新] {name} 第{page}页 API 请求失败: {e}")
                break
            except Exception as e:
                logger.error(f"[全量更新] {name} 第{page}页 处理异常: {type(e).__name__}: {e}")
                break

        codes.extend(market_codes)
        logger.info(f"[全量更新] {name} 共获取 {len(market_codes)} 只股票")

    codes = sorted(set(codes))
    valid_codes = [c for c in codes if c.isdigit() and len(c) == 6]

    logger.info(f"[全量更新] 东方财富API总共获取到 {len(valid_codes)} 只有效股票代码")
    return valid_codes
...
def _load_codes_from_cache() -> Optional[List[str]]:
    ...
    try:
        ...
        if time.time() - timestamp > _CACHE_TTL:
            logger.info("[全量更新] 股票代码缓存已过期")
            return None

        if not isinstance(codes, list) or len(codes) == 0:
            return None

        logger.info(f"[全量更新] 从缓存加载 {len(codes)} 只股票代码")
        return codes

    except Exception as e:
        logger.warning(f"[全量更新] 读取股票代码缓存失败: {e}")
        return None
...
def _save_codes_to_cache(codes: List[str]) -> None:
    ...
    try:
        ...
        logger.info(f"[全量更新] 已缓存 {len(codes)} 只股票代码")
    except Exception as e:
        logger.warning(f"[全量更新] 保存股票代码缓存失败: {e}")
...
def get_all_stock_codes() -> List[str]:
    ...
    if cached_codes:
        logger.info(f"[全量更新] 从缓存加载 {len(cached_codes)} 只股票代码")
        return cached_codes

    # 2. 优先从新浪 API 获取（更稳定）
    logger.info("[全量更新] 开始从新浪财经 API 获取股票代码列表...")

    try:
        codes = _fetch_stock_codes_from_sina()
        logger.info(f"[全量更新] 新浪财经 API 返回 {len(codes)} 只股票代码")

        if codes and len(codes) > 100:  # 确保获取到了有效数据
            _save_codes_to_cache(codes)
            logger.info(f"[全量更新] 成功获取并缓存 {len(codes)} 只股票代码")
            return codes
        else:
            logger.warning(f"[全量更新] 新浪 API 返回数据过少({len(codes)}条)，尝试东方财富")
    except Exception as e:
        logger.error(f"[全量更新] 从新浪获取股票代码失败: {e}")

    # 3. 尝试东方财富 API
    logger.info("[全量更新] 尝试从东方财富 API 获取...")
    try:
        codes = _fetch_stock_codes_from_eastmoney()
        logger.info(f"[全量更新] 东方财富 API 返回 {len(codes)} 只股票代码")
        ...
        if codes and len(codes) > 100:
            ...
            logger.warning(f"[全量更新] 东方财富 API 返回数据过少({len(codes)}条)")
    except Exception as e:
        logger.error(f"[全量更新] 从东方财富获取股票代码失败: {e}")
    ...
    try:
        if os.path.exists(_STOCK_CODES_CACHE_FILE):
            ...
            if codes and len(codes) > 10:
                logger.warning(f"[全量更新] 使用过期缓存数据: {len(codes)} 只股票")
                return codes
    except Exception as e:
        logger.warning(f"[全量更新] 读取过期缓存失败: {e}")

    # 5. 返回默认的模拟数据（兜底）
    logger.warning("[全量更新] 所有获取方式失败，返回默认股票列表（仅10条）")
