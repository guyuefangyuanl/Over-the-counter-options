...
from services.inquiry_status import InquiryStatus, InquiryStatusMachine, INQUIRY_STATUS_VALUES
...
def _sanitize_inquiry_data(data: Dict[str, Any], user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    sanitized = {
        ...
        "phone": data.get('contactPhone') or data.get('phone', ''),
...
def create_inquiry():
    ...
    try:
        ...
        if inquiry_id:
            logger.info(f"询价创建成功: id={inquiry_id}, userId={user_id or 'anonymous'}")
            return flask_success_response(data={"id": inquiry_id}, message="询价提交成功")
        else:
            logger.error("询价创建失败: 数据库返回空ID")
...
def admin_update_inquiry_status(id):
    ...
    try:
        ...
        if transition_error:
            logger.warning(f"状态流转验证失败: inquiry={id}, {current_status} -> {new_status}")
        ...
        if success:
            logger.info(f"询价状态更新成功: id={id}, {current_status} -> {new_status}")
...
def admin_batch_update_inquiries():
    ...
    try:
        ...
        logger.info(f"批量更新询价状态: status={new_status}, success={success_count}, failed={failed_count}")
