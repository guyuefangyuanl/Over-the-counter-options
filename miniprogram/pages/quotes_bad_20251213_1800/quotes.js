// miniprogram/pages/quotes/quotes.js
const FAVORITES_STORAGE_KEY = 'INQUIRY_FAVORITES_V1';
const api = require('../../utils/api.js');

// 尝试引用 inquiry-logic.js，如果路径不同需要调整
...
Page({
  ...
  fetchQuoteList() {
    ...
    db.collection('quotes').limit(20).get().then(res => {
      ...
      const mockList = Array.from({ length: 10 }).map((_, i) => ({
        ...
        term: '1M',
        structure: 'vanilla',
        rates: { '100': (10 + i * 0.5).toFixed(2) }
      }));
      this.setData({ _fullQuoteList: mockList });
      this.filterQuoteList();
  ...
  computeGroupCounts() {
    const { favoritesById } = this.data;
    // 如果 logic 模块不可用，提供降级处理
    const counts = logic && logic.computeGroupCountsFromFavorites 
      ? logic.computeGroupCountsFromFavorites(favoritesById)
      : {};
  ...
  goToDetail(e) {
    const id = (e && e.detail && e.detail.id) !== undefined ? e.detail.id : (e.currentTarget && e.currentTarget.dataset ? e.currentTarget.dataset.id : undefined);
    wx.navigateTo({
      url: `/pages/quotes/detail/detail?id=${id}`
    });
  },
  ...
  onAddFavoritesSearchConfirm() {
    ...
    if (match) {
      wx.navigateTo({ url: `/pages/quotes/detail/detail?id=${match.id}` });
    } else {
  ...
  openOptionQuote(e) {
    ...
    if (id !== undefined && id !== null && id !== '') {
      wx.navigateTo({ url: `/pages/quotes/detail/detail?id=${id}` });
    }
  },
