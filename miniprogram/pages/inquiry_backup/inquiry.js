...
const GROUPS_STORAGE_KEY = 'INQUIRY_GROUPS_V1';
...
Page({
  ...
  loadGroups() {
    try {
      const groups = wx.getStorageSync(GROUPS_STORAGE_KEY);
      if (groups) {
  ...
  saveGroups() {
    try {
      wx.setStorageSync(GROUPS_STORAGE_KEY, this.data.customGroups);
    } catch (e) {
  ...
  onNewGroupInput(e) {
    const name = e.detail;
    const { error, canConfirm } = logic.validateNewGroupName(name, this.data.customGroups);
  ...
  confirmNewGroup() {
    ...
    setTimeout(() => {
      ...
      // 新建分组插入到最前面（按创建时间倒序）
      const customGroups = [newGroup, ...this.data.customGroups];
      this.setData({ 
        customGroups,
        showNewGroupDialog: false,
        newGroupLoading: false
  ...
  onRenameGroupInput(e) {
    const name = e.detail;
    const { error, canConfirm } = logic.validateRenameGroupName(name, this.data.editingGroupId, this.data.customGroups);
  ...
  openDeleteGroupDialog(e) {
    ...
    // 计算需要迁移的自选数量
    const { favoritesById, customGroups, systemGroups } = this.data;
    const migrationCount = logic.computeMigrationCount(id, favoritesById);
    
    // 获取可用于迁移的目标分组
    const availableTargetGroups = logic.getAvailableTargetGroups(id, systemGroups, customGroups);
  ...
  confirmDeleteGroup() {
    ...
    if (deleteMigrationCount > 0 && availableTargetGroups.length > 0) {
      ...
      updatedFavorites = logic.migrateGroupItems(editingGroupId, targetGroupId, favoritesById);
    } else {
      // 直接删除该分组的自选
